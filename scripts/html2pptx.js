...
function addElements(slideData, targetSlide, pres) {
  for (const el of slideData.elements) {
    if (el.type === 'image') {
      ...
      // Make single-line text 10% wider to absorb font-fallback metric drift
      // (SUIT in browser vs PowerPoint fallback like Malgun Gothic)
      if (isSingleLine) {
        const widthIncrease = Math.max(el.position.w * 0.10, 24);
      ...
      const textOptions = {
        ...
        wrap: !isSingleLine,       // multi-line text: wrap inside box; single-line: no wrap
        autoFit: false             // never shrink-to-fit; prefer width buffer
      };
